...
class Dispatcher extends Object
{
     ...
     */
    public $targets = [['class' => 'Swheel\Log\EchoTarget']];
    ...
    public function __construct($config = [])
    {
        ...
        if (isset($config['logger'])) {
            ...
            unset($config['logger']);
            $this->setTraceLevel($this->traceLevel);
        }
    ...
    public function init()
    {
        ...
        foreach ($this->targets as $name => $target) {
            if (!$target instanceof Target) {
                $this->targets[$name] = Object::createObject($target);
            }
        }
    }
    ...
    public function getLogger()
    {
        if ($this->_logger === null) {
            $this->setLogger(Object::createObject('Swheel\Log\Logger'));
        }
        return $this->_logger;
    }
    ...
    public function setLogger($value)
    {
        if (is_string($value) || is_array($value)) {
            $value = Object::createObject($value);
        }
        $this->_logger = $value;
